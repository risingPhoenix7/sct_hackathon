...
class TSPApproximation:
    def tspApproximation(self, graph, V):
        # Find the Minimum Spanning Tree using Prim's Algorithm
        parent = self.primMST(graph, V)

        # Find preorder traversal of the MST to approximate Hamiltonian cycle
        visited = [False] * V
        path = []
        self.findPreorderTraversal(parent, 0, visited, path, V)
        # Adding the start node at the end to complete the cycle
        path.append(0)
    ...
    def findMinKey(self, key, mstSet, V):
        min_val = float('inf')
        min_index = -1
        for v in range(V):
    ...
    def primMST(self, graph, V):
        ...
        for _ in range(V - 1):
            u = self.findMinKey(key, mstSet, V)
            mstSet[u] = True
    ...
    def findPreorderTraversal(self, parent, node, visited, path, V):
        visited[node] = True
        path.append(node)
        for i in range(V):
            if parent[i] == node and not visited[i]:
                self.findPreorderTraversal(parent, i, visited, path, V)
